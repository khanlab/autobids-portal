...
def upgrade():
    ...
    with op.batch_alter_table('tar2bids', schema=None) as batch_op:
        batch_op.add_column(sa.Column('tar_file_id', sa.Integer(), nullable=True))
        batch_op.add_column(sa.Column('bids_file', sa.String(length=200), nullable=True))
        batch_op.create_index(batch_op.f('ix_tar2bids_bids_file'), ['bids_file'], unique=False)
